...
def get_median_ls(X, Y=None):
    ...
    with torch.no_grad():
        ...
        ret = torch.sqrt(
            torch.median(d[d >= 0])
        )  # print this value, should be increasing with d
        if ret.item() == 0:
...
class train_krr_simple:
    ...
    def load_and_split_data(self):
        ...
        tr_ind, val_ind, test_ind = StratifiedKFold3(
            n_splits=self.folds, shuffle=True, random_state=self.seed
        ).split(indices, y)[self.fold]
    ...
    def SGD_krr_loop(self):
        ...
        best_model, best_tr, best_val, best_test = self.model(
            train_X, train_y, val_X, val_y, test_X, test_y, inp, 1e-5, self.m
        )
    ...
    def train_model(self):
        ...
        pickle.dump(model_copy, open(self.save_dir + f"run_{self.fold}.pickle", "wb"))
    ...
    def load_and_split_data(self):
        ...
        if self.model_string in ["SGD_ukrr", "krr_user", "SGD_ukrr_pgp"]:
            ...
            self.X_val = np.concatenate(
                [self.val_u, self.left_val, self.right_val], axis=1
            )
            self.X_test = np.concatenate(
                [self.test_u, self.left_test, self.right_test], axis=1
            )
        else:
    ...
    def SGD_krr_loop(self):
        ...
        if self.model_string in ["SGD_ukrr", "SGD_ukrr_pgp"]:
            ...
            results = {
                ...
                "ls": best_model.kernel.sigma.detach().cpu()
                if self.model_string == "SGD_base"
                else best_model.kernel.lengthscale.detach().cpu(),
                "lamb": best_model.penalty.detach().cpu(),
                "alpha": alpha.cpu(),
                "inducing_points": best_model.centers.detach().cpu(),
            }
