...
def produce_plots_pokemon_local(dirname, data_name, fn, d, max_disp=7):
    ...
    for lamb in piv_df["lambda"].unique().tolist():
        ...
        shap_values = shap.Explanation(
            values=vals, feature_names=feature_cols, data=data[cols].values
        )
        fig = plt.gcf()
        shap.plots.beeswarm(
            shap_values, max_display=max_disp, show=False, color_bar=False
        )
