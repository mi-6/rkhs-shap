...
def train_krr(kernel, Xtrain, Ytrain, Xval, Yval, Xtest, Ytest, pen, m_fac=1.0):
    ...
    centers_init = Xtrain[
        np.random.choice(Xtrain.shape[0], size=(M,), replace=False)
    ].clone()
    model = NystromCompRegSHAP(
        kernel=kernel,
        penalty_init=penalty_init,
        centers_init=centers_init,  # The initial hp values
        opt_penalty=True,
        opt_centers=True,  # Whether the various hps are to be optimized
    )
...
def SGD_KRR_base(Xtrain, Ytrain, Xval, Yval, Xtest, Ytest, ls, pen=1e-5, m_fac=1.0):
    ...
    kernel = GaussianKernel(
        sigma=lengthscale_init, opt=FalkonOptions(keops_active="yes")
    )
...
def SGD_KRR(Xtrain, Ytrain, Xval, Yval, Xtest, Ytest, ls, pen=1e-5, m_fac=1.0):
    ...
    kernel = diffrentiable_FALKON_GPGP(
        lengthscale=lengthscale_init, options=falkon.FalkonOptions()
    )
    return train_krr(kernel, Xtrain, Ytrain, Xval, Yval, Xtest, Ytest, pen, m_fac)
...
def SGD_KRR_PGP(Xtrain, Ytrain, Xval, Yval, Xtest, Ytest, ls, pen=1e-5, m_fac=1.0):
    lengthscale_init = torch.tensor([ls] * (Xtrain.shape[1] // 2)).requires_grad_()
    kernel = diffrentiable_FALKON_PGP(
        lengthscale=lengthscale_init, options=falkon.FalkonOptions()
    )
    return train_krr(kernel, Xtrain, Ytrain, Xval, Yval, Xtest, Ytest, pen, m_fac)
...
def SGD_UKRR(Xtrain, Ytrain, Xval, Yval, Xtest, Ytest, dat, pen=1e-5, m_fac=1.0):
    ...
    lengthscale_i = torch.tensor(
        [ls_i] * ((Xtrain.shape[1] - user_dim) // 2)
    ).requires_grad_()
    lengthscale_u = torch.tensor([ls_u] * user_dim).requires_grad_()
    kernel = diffrentiable_FALKON_UGPGP(
        lengthscale_items=lengthscale_i,
        lengthscale_users=lengthscale_u,
        user_dim=user_dim,
...
def SGD_UKRR_PGP(Xtrain, Ytrain, Xval, Yval, Xtest, Ytest, dat, pen=1e-5, m_fac=1.0):
    ...
    lengthscale_u = torch.tensor([ls_u] * user_dim).requires_grad_()
    kernel = diffrentiable_FALKON_UPGP(
        lengthscale_items=lengthscale_i,
        lengthscale_users=lengthscale_u,
        user_dim=user_dim,
        options=falkon.FalkonOptions(),
...
class diffrentiable_FALKON_GPGP(DiffKernel):
    def __init__(self, lengthscale, options):
        ...
        super().__init__(
            "diffrentiable_FALKON_GPGP", options, core_fn=None, lengthscale=lengthscale
        )

    def compute(
        self, X1: torch.Tensor, X2: torch.Tensor, out: torch.Tensor, diag: bool
    ):
        ls = self.lengthscale
        ...
        if diag:
            ...
            K = (-((xa_ - xc_) ** 2 + (xb_ - xd_) ** 2) / 2).sum(-1).exp() - (
                -((xa_ - xd_) ** 2 + (xb_ - xc_) ** 2) / 2
            ).sum(-1).exp()
            out.copy_(K)
        else:
            K = (
                -(pairwise_distances(xa_, xc_) + pairwise_distances(xb_, xd_)) / 2
            ).exp() - (
                -(pairwise_distances(xa_, xd_) + pairwise_distances(xb_, xc_)) / 2
            ).exp()
            out.copy_(K)
        return out
    ...
    def detach(self):
        ...
        return diffrentiable_FALKON_GPGP(
            lengthscale=self.lengthscale.detach(), options=self.params
        )
...
class diffrentiable_FALKON_UPGP(DiffKernel):
    ...
    def compute(
        ...
        if diag:
            K = (
                (-((xa_ - xc_) ** 2) / 2).sum(-1).exp()
                + (-((xb_ - xd_) ** 2) / 2).sum(-1).exp()
                - (-((xa_ - xd_) ** 2) / 2).sum(-1).exp()
                - (-((xb_ - xc_) ** 2) / 2).sum(-1).exp()
            )
            L = K * (-((u_1_ - u_2_) ** 2)).sum(-1).exp()
            out.copy_(L)
        else:
            K = (
                (-pairwise_distances(xa_, xc_) / 2).exp()
                + (-pairwise_distances(xb_, xd_) / 2).exp()
                - (-pairwise_distances(xa_, xd_) / 2).exp()
                - (-pairwise_distances(xb_, xc_) / 2).exp()
            )
            L = K * (-pairwise_distances(u_1_, u_2_)).exp()
            out.copy_(L)
        return out
...
class diffrentiable_FALKON_PGP(DiffKernel):
    def __init__(self, lengthscale, options):
        ...
        # but we must specify the hyperparameter of this kernel (lengthscale)
        super().__init__(
            "diffrentiable_FALKON_PGP", options, core_fn=None, lengthscale=lengthscale
        )
