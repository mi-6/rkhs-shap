...
def elastic_regression(l1_weight, l2_weight, X, w, y):
    ...
    dataloader_train = DataLoader(
        dataset_train, batch_size=X_tr.shape[0] // 10, shuffle=True
    )
    dataset_test = TensorDataset(X_te, y_te)
    dataloader_test = DataLoader(dataset_test, batch_size=X_tr.shape[0], shuffle=True)
    trainer = pl.Trainer(
        max_epochs=100, accelerator="gpu", devices=[0], callbacks=early_stop_callback
    )
...
class pref_shap:
    ...
    def setup_the_rest(self):
        self.tensor_CG = tensor_CG(
            precond=self.precond,
            reg=self.reg,
            eps=self.eps,
            maxits=self.cg_max_its,
            device=self.device,
        )
        ...
        abs_S = Z.sum(1)
        a = torch.exp(
            const - torch.lgamma((self.eff_dim - abs_S) + 1) - torch.lgamma(abs_S + 1)
        )
        ...
        Z = torch.concat(
            [
                torch.zeros(1, self.eff_dim).to(self.device),
                middle_Z,
                torch.ones(1, self.eff_dim).to(self.device),
            ],
            dim=0,
        )
        ...
        self.batched_Z = torch.chunk(
            self.Z, max(self.Z.shape[0] // self.cg_bs, 1), dim=0
        )
    ...
    def kernel_tensor_batch(self, S_list_batch, x, x_prime):
        ...
        output = (cat_xls_xs * cat_xrs_xs_prime) * (cg_l_a * cg_r_a) - (
            cat_xls_xs_prime * cat_xrs_xs
        ) * (cg_l_b * cg_r_b)
    ...
    def kernel_tensor_batch_pgp(self, S_list_batch, x, x_prime):
        ...
        cat_xrs_xs = torch.stack(cat_xrs_xs, dim=0)
        cat_xrs_xs_prime = torch.stack(cat_xrs_xs_prime, dim=0)
        output = (cat_xls_xs * cg_l_a + cat_xrs_xs_prime * cg_r_a) - (
            cat_xls_xs_prime * cg_l_b + cat_xrs_xs * cg_r_b
        )
        return output, first_flag, last_flag
    ...
    def kernel_tensor_batch_user_case_1(self, u, S_batch, x, x_prime):
        ...
        for i in range(S_batch.shape[0]):
            ...
            if S.sum() == 0:
                ...
                RH_cat = self.k(self.X_l, x) * self.k(self.X_r, x_prime) - self.k(
                    self.X_l, x_prime
                ) * self.k(self.X_r, x)
                RH.append(RH_cat)
                vec.append(vec_cat)
    ...
    def kernel_tensor_batch_user_case_2(self, u, S_list_batch, x, x_prime):
        output, first_flag, last_flag = self.kernel_tensor_batch(
            S_list_batch, x, x_prime
        )
    ...
    def value_observation(self, S_batch, x, x_prime, u=None, case=2, pgp=False):
        if u is None:
            if pgp:
                output, first_flag, last_flag = self.kernel_tensor_batch_pgp(
                    S_batch, x, x_prime
                )
            else:
                output, first_flag, last_flag = self.kernel_tensor_batch(
            ...
            if case == 1:
                output, first_flag, last_flag = self.kernel_tensor_batch_user_case_1(
                    u, S_batch, x, x_prime
                )
            elif case == 2:
                output, first_flag, last_flag = self.kernel_tensor_batch_user_case_2(
                    u, S_batch, x, x_prime
                )
