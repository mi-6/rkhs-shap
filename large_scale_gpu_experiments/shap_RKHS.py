...
def cumsum_thingy_2(cumsum_indices, shapley_vals):
    if cumsum_indices:
        ...
        for i in range(len(cumsum_indices) - 1):
            part = shapley_vals[:, cumsum_indices[i] : cumsum_indices[i + 1]].sum(
                1, keepdim=True
            )
...
def get_shapley_vals(
    job, model_string, fold, train_params, num_matches, post_method, interventional
):
...
def get_shapley_vals_2(cooking_dict, job, post_method, m="SGD_krr"):
    sum_count, features_names, do_sum, coeffs = return_feature_names(job)
    outputs = construct_values(
        cooking_dict["Y"],
        cooking_dict["Z"],
        cooking_dict["weights"],
        coeffs,
        post_method,
    )
    big_plot = []
    for key, output in outputs.items():
        ...
        plot = pd.DataFrame(
            np.stack([tst, tmp, np.ones_like(tst) * key], axis=1),
            columns=["d", "shapley_vals", "lambda"],
        )
            ...
            for f in [0]:
                ...
                for post_method in ["lasso"]:
                    ...
                    for f in [0]:
                        ...
                        data, features_names = get_shapley_vals_2(
                            cooking_dict, job, post_method, m
                        )
                        data["fold"] = f
                        big_plt.append(data)
