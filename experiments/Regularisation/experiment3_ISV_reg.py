...
def fair_learning_with_rkhs_shap(
    X,
    y,
    lambda_sv,
    lambda_krr,
    ...
    features_index,
    sv_method_to_train,
    sv_method_to_evaluate,
):
    krr_reg = ShapleyRegulariser(
        lambda_sv=lambda_sv,
...
def run_attack_experiment(n=1300, m=5, noise_ls=[0], sv_reg_ls=[0], verbose=True):
    ...
    # Linear model
    clm_obj = CorrelatedLinearModel(
        n=n, noise=3, mean_vec=mean_vec, Sigma=Sigma, beta=beta
    )
    ...
    for noise in noise_ls:
        for sv_reg in sv_reg_ls:
            ...
            B_reg, train_rmse, new_krr = fair_learning_with_rkhs_shap(
                X=X_train,
                y=y_train,
                lambda_sv=sv_reg,
                lambda_krr=krr.lmda,
...
sv_reg_ls = [0, 1e-1, 0.5, 1, 1.5, 2, 2.5, 5.0]

for round in range(repetition):
    test_rmse_ls, test_rmse_on_attack_ls, rmse_ratio_ls = run_attack_experiment(
        n=1300, m=5, noise_ls=noise_ls, sv_reg_ls=sv_reg_ls
    )
    result = [
        noise_ls,
        sv_reg_ls,
        test_rmse_ls,
        test_rmse_on_attack_ls,
        rmse_ratio_ls,
        round,
    ]
